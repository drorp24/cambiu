...
$(document).ready(function() {
    ...
    pageSwitch = function(old_page, new_page) {
        ...
        history.pushState(new_page.attr('id'), null, new_page.attr('id'));
        console.log('push. saved state: ' + new_page.attr('id'))
        new_page.show();
    };

    window.addEventListener("popstate", function(e) {
        console.log('pop. e.state: ' + e.state)
        state =  (e.state) ? e.state : 'exchanges'  ;
        old_page = $('#' + location.pathname.replace('/', ''));
        new_page = $('#' + state);
