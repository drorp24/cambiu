...
=end
  ...
  def self.update(chain_name=nil, exchange_name=nil, url)
    ...
    if url == "https://www.eurochange.co.uk/exchangerates.aspx"
      klass = '.ERTable'
    else
      klass = ''
    end

    raise "No tbody element" unless tbody = doc.css('table tbody' + klass)
...
puts tr
  ...
  def self.parse_rates(url, tr)
    ...
    if url == "https://www.bfcexchange.co.uk/en/rates.html"
      ...
      result[:currency]   =
          case
            when currency_name == 'Japanese Yen'
              'JPY'
            when currency_name == 'Canadian Dollar'
              'CAD'
            when currency_name == 'Australian Dollar'
              'AUD'
            when currency_name == 'US Dollar'
              'USD'
            when currency_name == 'Euro'
              'EUR'
          end
      result[:buy]        =  tr.css('td')[1].text.strip
      result[:sell]       =  tr.css('td')[2].text.strip
      puts result.inspect
      ...
      result[:currency]   =
          case
            ...
            when currency_name == 'Euro'
              'EUR'
            when currency_name == 'Israeli Shekels'
              'ILS'
            when currency_name == 'Hong Kong Dollar'
              'HKD'
            when currency_name == 'Chinese Yuan'
              'CNY'
          end
      result[:buy]        =  tr.css('td')[1].text.strip
      result[:sell]       =  nil
